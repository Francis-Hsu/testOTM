...
An important result, known as Brenier-McCann's Theorem [@B1991; @M1995], implies that there exists a piecewise affine function $\hat{\psi}_n$ such that $\hat{Q}_n=\nabla\hat{\psi}_n$. Since the rank function in 1D is the inverse of the quantile map, the multivariate empirical rank function can be defined as \[\hat{R}_n(\mathbf{x})=\operatorname*{arg\,min}_{\mathbf{y}\in\mathcal{Y}}\{\mathbf{x}^\top\mathbf{y}-\hat{\psi}_n(\mathbf{y})\},\] and it can be computed through linear programming. This definition is justified by the inversion rule of sub-gradient: \[\mathbf{x}\in\partial\hat{\psi}_n(\mathbf{y})\iff\mathbf{y}\in\partial\hat{\psi}{}_n^\ast(\mathbf{x}),\] where $\hat{\psi}{}_n^\ast$ is the Legendre-Fenchel dual of $\hat{\psi}_n$.
